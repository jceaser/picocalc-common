...
// ScrollUp shifts the screen up by the specified number of pixels
...
// ScrollDown shifts the screen down by the specified number of pixels
func (disp *Ili948x) ScrollDown(pixels int16) {
	disp.ScrollUp(-pixels)
}

// ResetScroll returns to the normal (non-scrolled) view
func (disp *Ili948x) ScrollReset() {
	disp.ScrollUp(0)
}

//func (disp *Ili948x) FillRectangle(x, y, width, height int16, c RGB565) {

func (disp *Ili948x) ScrollUpClear(pixels int16, color RGB565) {
	disp.ScrollUp(pixels)
	disp.FillRectangle(0, 320-pixels, 320, pixels, color)
}
